...
func (cfg *Config) InitFromFlags(fs *flag.FlagSet) {
	...
	flag.BoolVar(&cfg.IgnoreOperationAnnotation, ignoreOperationAnnotationFlagName, defaultIgnoreOperationAnnotation,
		fmt.Sprintf("Specifies whether to ignore or honour the annotation `%s: %s` on resources to be reconciled. Deprecated: please use `--%s` instead.", druidv1alpha1.DruidOperationAnnotation, druidv1alpha1.DruidOperationReconcile, enableEtcdSpecAutoReconcileFlagName))
